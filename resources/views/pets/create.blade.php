    ...
    <div class="card text-white md:w-[720px] w-[320px] bg-black/20 p-4 mb-4 rounded">
        <form method="POST" action="{{ url('pets') }}" class="flex flex-col md:flex-row gap-4 mt-4" enctype="multipart/form-data">
            ...
            <div class="w-full md:w-[320px]">
                ...
                <input type="text" class="input bg-[#0009] outline-0" name="name" placeholder="Max" value="{{ old('name') }}">
                @error('name')
                    <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                @enderror
                {{-- Kind --}}
                ...
                <input type="text" class="input bg-[#0009] outline-0" name="breed" placeholder="Labrador Retriever" value="{{ old('breed') }}">
                @error('breed')
                    <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                @enderror
                {{-- Location --}}
            ...
            <div class="w-full md:w-[320px]">
                ...
                <input type="number" step="0.1" class="input bg-[#0009] outline-0" name="weight" placeholder="5.2" value="{{ old('weight') }}">
                @error('weight')
                    <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                @enderror
                {{-- Age --}}
                ...
                <textarea class="textarea bg-[#0009] outline-0 w-full" name="description" placeholder="Friendly and playful...">{{ old('description') }}</textarea>
                @error('description')
                    <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                @enderror
